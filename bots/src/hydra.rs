...
pub(crate) fn choose_hydra_move(game: &Game) -> Result<[Cube; 2], String> {
    ...
    let raw_pairs = pairs.iter().map(|entry| entry.pair).collect::<Vec<_>>();
    ...
    if !forced_blocks.is_empty() {
        let filtered = filter_pairs_covering_threats(&raw_pairs, &forced_blocks);
        if !filtered.is_empty() {
            let allowed = filtered.iter().copied().collect::<FxHashSet<_>>();
            pairs.retain(|entry| allowed.contains(&entry.pair));
        }
    }
...
fn filter_pairs_covering_threats(
    pairs: &[[Cube; 2]],
    threats: &[WindowPattern],
) -> Vec<[Cube; 2]> {
    pairs
        .iter()
        .copied()
