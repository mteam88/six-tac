...
export function createSessionController(options: {
  ...
  const updateLobbyButtons = (): void => {
    ...
    for (const button of [elements.localModeButton, elements.createRoomButton, elements.playBotButton, elements.joinRoomButton]) {
  ...
  const leaveRoom = (): void => {
    ...
    if (!confirmed) return;
    stopPolling();
    session.mode === "local" ? persistLocalSession() : persistSession(null);
    showLobby("");
  };
